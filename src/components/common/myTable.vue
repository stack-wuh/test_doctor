  ...
  <section class="wrapper">
    ...
    <el-table :data="list" border  stripe @selection-change="handleSelectionChange" >
      ...
      <el-table-column v-if="item.type === 'switch'" align="center" v-for="(item,index) in tableList.list" :key="index" :label="item.key" :prop="item.prop" :width="item.width || ''">
        <template slot-scope="scope">
          <el-switch v-model="scope.row[item.prop]" @change="item.change(routers,scope.row, scope.$index)" active-text="启用" inactive-text="禁用" inactive-color="#999999" :active-value="1" :inactive-value="0" ></el-switch>
        </template>
      </el-table-column>
