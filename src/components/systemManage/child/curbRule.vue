...
export default {
  ...
  watch: {
    ...
    changePath(){
      if(this.changePath === '关注后消息推送'){
        this.getPushInfo()
      }else if(this.changePath === '会员卡积分规则'){
       this.fetchDataForIntegral()
      }
    }
  },
  methods: {
    ...
    handleSubmitIntegralRule() {
      ...
      this.$http.post(
        "platform/addOrUpIntegralReturn.do",
        { data: form },
        res => {
          this.getInterList();
        }
    ...
    handleSubmitToCash() {
      ...
      this.setCouponMemberSells({
        form: { ids: ids.toString(), tocashs: tocashs.toString() }
      });
    },

    fetchDataForIntegral(){
      this.getInterList();
      this.getInterListRules();
      this.getPushList();
